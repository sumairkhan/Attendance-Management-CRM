<%@ WebHandler Language="VB" Class="scales" %>
...
Public Class clsScales
    Public Property ScaleID() As System.Nullable(Of Integer)

    Public Property Title() As String
End Class


Public Class scales : Implements IHttpHandler
    ...
    Private Sub getList(ByVal context As HttpContext)
        Try
            Using cntx As New PortalModel.PortalEntities
                Dim obj = (From b In cntx.HR_Scales Where b.IsDeleted = False Order By b.Title Select b.ScaleID, b.Title).ToList
    ...
    Private Sub edit(ByVal context As HttpContext, ByVal intID As Integer)
        ...
        Try
            ...
            Dim objTbl = JsonConvert.DeserializeObject(Of List(Of clsScales))(JsonStr)

            Using cntx As New PortalModel.PortalEntities
                Dim strName As String = objTbl(0).Title.Trim().ToLower()
                Dim intObjID As Integer = objTbl(0).ScaleID
                Dim objTblExist = (From tbl In cntx.HR_Scales Where tbl.IsDeleted = False AndAlso tbl.Title.Trim().ToLower() = strName AndAlso tbl.ScaleID <> intObjID Select tbl).FirstOrDefault

                If objTblExist Is Nothing Then
                    Dim obj = cntx.HR_Scales.Where(Function(f) f.ScaleID = intObjID).ToList().FirstOrDefault()
                    Dim objOld = New PortalModel.PortalEntities().HR_Scales.Where(Function(f) f.ScaleID = intObjID).ToList().FirstOrDefault()
                    cntx.HR_Scales.Attach(obj)
                    With obj
                    ...
                    Try
                        PortalUtilities.fnCompare(CType(objOld, Object), CType(obj, Object), obj.ScaleID, lstEmployeeAndCompanyID(0))
                    Catch ex As Exception
    ...
    Private Sub add(ByVal context As HttpContext, ByVal intID As Integer)
        ...
        Try
            ...
            Using cntx As New PortalModel.PortalEntities
                Dim strName As String = objTbl(0).Title.Trim().ToLower()
                Dim objDesignation = (From tbl In cntx.HR_Scales Where tbl.IsDeleted = False AndAlso tbl.Title.Trim().ToLower() = strName Select tbl).FirstOrDefault

                If objDesignation Is Nothing Then
                    Dim obj = New PortalModel.HR_Scales
                    With obj
                        .Title = objTbl(0).Title.Trim()
    ...
    Private Sub delete(ByVal context As HttpContext, ByVal intID As Integer)
        ...
        Try
            ...
            Dim JsonStr = context.Request("models")
            Dim Dept = JsonConvert.DeserializeObject(Of List(Of clsScales))(JsonStr)

            Using cntx As New PortalModel.PortalEntities
                Dim obj = New PortalModel.HR_Scales With {.ScaleID = Dept(0).ScaleID}
